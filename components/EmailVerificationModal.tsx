...
import { X, Mail, ShieldCheck, Loader2, ArrowRight, CheckCircle2, AlertCircle } from 'lucide-react';
...
const EmailVerificationModal: React.FC<EmailVerificationModalProps> = ({ onClose, user, onSuccess }) => {
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      if (data.emailVerified || data.fullyVerified) {
      ...
      <div className="bg-white w-full max-w-md rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
        ...
        <div className="p-8 pt-6">
          <h2 className="text-2xl font-black text-slate-900 tracking-tight mb-2">
            {step === 'input' ? 'Verify Email' : 'Enter Code'}
          </h2>
          <p className="text-sm font-medium text-slate-500 leading-relaxed mb-8">
            {step === 'input' 
              ? 'We need to verify your email address to ensure secure transactions and real-time updates.'
